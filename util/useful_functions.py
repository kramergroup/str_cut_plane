...
def load_stream(stream, comm_char="#", f=lambda x: x):
    ...
    comments = [l.split() for l in tmp_in if l[0] == comm_char]
    tmp_in = [[f(x) for x in l.split()]
              for l in tmp_in if l[0] != comm_char]
    return comments, tmp_in

def load_file(filename, comm_char="#"):
    ...
    with open(filename, 'r') as in_file:
        return load_stream(in_file, comm_char)
    
def load_float_file(filename, comm_char="#"):
    """Load given numeric filename: each non-comment field will be cast to float. Divide lines between comments and data.
    Comment char can be given as input. Wrapped of load_stream function."""
    # TODO Regex support for multiple comment types.
    with open(filename, 'r') as in_file:
        # Appreciate the power of **kwarg expansion!
        return load_stream(in_file, **{"comm_char" : comm_char, "f": float})
